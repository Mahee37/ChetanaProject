    ...
    <div class="row">
        <div class="col-md-4">
            <c:choose>
                <c:when test="${not empty productdetails.product.imageUrl}">
                    <img src="${productdetails.product.imageUrl}" alt="${productdetails.product.productName}" class="img-fluid mb-4">
                </c:when>
                <c:otherwise>
        ...
        <div class="col-md-8">
            <c:choose>
                <c:when test="${not empty productdetails.product}">
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Category:</strong> ${productdetails.product.productCategory}</p>
                            <p class="card-text"><strong>Price:</strong> &#8377;${productdetails.product.price}</p>
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <form method="post" action="${pageContext.request.contextPath}/buyer/addProductsToCarts?id=${productdetails.product.productId}">
                            ...
                            <!-- New button to check the cart -->
                            <form action="${pageContext.request.contextPath}/BuyerCart" method="get">
                                <button type="submit" class="btn-orange mt-3">Check Cart</button>
                            </form>

                            <c:if test="${not empty productdetails.seller}">
                                <div class="mt-4">
                                    <h4>Seller Information</h4>
                                    <p><strong>Name:</strong> ${productdetails.seller.name}</p>
                                    <p><strong>Address:</strong> ${productdetails.seller.address}</p>
                                </div>
                            </c:if>
                            ...
                            <c:choose>
                                    ...
                                    <form method="post" action="${pageContext.request.contextPath}/reviews">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" id="rating1" name="rating" value="1" required>
                                ...
                                <c:otherwise>
                                    <div class="alert alert-warning" role="alert">You must be logged in as a buyer to post a review.</div>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </div>
                ...
                <c:otherwise>
                    <div class="alert alert-danger" role="alert">Product details not found.</div>
                </c:otherwise>
            </c:choose>
        </div>
    </div>
